...
forPlotting <- df %>%
  ...
  mutate(
    se = logssd / sqrt(count),
    logse = ssd / sqrt(count),
...
print("no differences on completion so what takes least different gestures guesses to success?")

qqnorm(dfDetails$numOfGesturesTriedB4)
qqline(dfDetails$numOfGesturesTriedB4)
print("not the best way ;-) - Anyway here's the anova")

summary(aov(numOfGesturesTriedB4 ~ sigPlace * gesture, dfDetails))

print("and here's a better way to predict number of tried gestures")
